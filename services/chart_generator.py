...
class ChartGenerator:
    ...
    def _generate_boxer_pie_chart(filtered_data, selected_boxers, year, location, gender):
        ...
        for boxer in boxers_to_show:
            ...
            if not boxer_data.empty:
                ...
                if location == "All Locations":
                    label = f"<b>{boxer}</b><br>{gym_name} ({win_ratio:.1%})"
                else:
                    label = f"<b>{boxer}</b><br>{gym_name} ({win_ratio:.1%})"
        ...
        if pie_data:
            ...
            fig = px.pie(
                ...
                color='Label', # Always color by label to ensure uniqueness
                color_discrete_sequence=ChartGenerator._get_color_sequence("pie", "Boxer", location),
                hole=0.3 # Donut chart for better label fit
            )
    ...
    def _generate_gym_pie_chart(filtered_data, year, location):
        ...
        for gym in filtered_data['Gym'].unique():
            ...
            if location == "All Locations":
                display_name = f"<b>{gym}</b><br>({location_name}) {win_ratio:.1%}"
                color_by = 'Location'
            else:
                display_name = f"<b>{gym}</b><br>({win_ratio:.1%})"
                color_by = 'Gym'
        ...
        fig = px.pie(
            ...
            color='Gym', # Always color by gym label
